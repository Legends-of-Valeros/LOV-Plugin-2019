...
public class ThreatBehavior {
    ...
    public static final BehaviorAction FIND = new BehaviorAction() {
        ...
        public NodeStatus onStep(CombatEntity ce, long ticks) {
            ...
            List<Entity> entities = ce.getLivingEntity().getNearbyEntities(trait.instance.mob.distance.detection, trait.instance.mob.distance.detection, trait.instance.mob.distance.detection);
                ...
                if (entity instanceof LivingEntity && !NPCsController.getInstance().isNPC((LivingEntity) entity)) {
                    if (entity instanceof Player) {
                        ...
                        if (((Player) entity).isSneaking()) {
                            ...
                            if (entity.getLocation().distance(ce.getLivingEntity().getLocation()) > Math.round(trait.instance.mob.distance.detection / 2)) {
                                continue;
                            }
                        }
                    }
            ...
            if (target.getLocation().distanceSquared(ce.getLivingEntity().getLocation()) > 2 &&
                ...
                if (history != null) {
                    if (history.didDamage(target)) {
                        if (!trait.instance.mob.leashed.containsKey(characterId)) {
                            trait.instance.mob.leashed.put(characterId, System.currentTimeMillis() / 1000L);
                        } else {
                            long since = trait.instance.mob.leashed.get(characterId);
