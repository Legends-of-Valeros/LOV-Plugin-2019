...
public class StatDisplay {
  ...
  static {
    display.put(Stat.HEALTH_REGEN,
        () -> new String[]{"+" + DF.format(CombatEngine.getEngineConfig().getRegenPercentagePerPoint(RegeneratingStat.HEALTH) * 100) + "% rate"});
    display.put(Stat.MANA_REGEN,
        () -> new String[]{"+" + DF.format(CombatEngine.getEngineConfig().getRegenPercentagePerPoint(RegeneratingStat.MANA) * 100) + "% rate"});
    display.put(Stat.ENERGY_REGEN,
        () -> new String[]{"+" + DF.format(CombatEngine.getEngineConfig().getRegenPercentagePerPoint(RegeneratingStat.ENERGY) * 100) + "% rate"});
