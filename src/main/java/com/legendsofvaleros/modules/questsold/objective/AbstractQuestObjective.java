package com.legendsofvaleros.modules.questsold.objective;
...
public abstract class AbstractQuestObjective<T> implements IQuestObjective<T> {
    ...
    @Override public IQuest getQuest() {
        return quest.get();
    }

    private int groupI;

    @Override public int getGroupIndex() {
        return groupI;
    }

    private int objectiveI;

    @Override public int getObjectiveIndex() {
        return objectiveI;
    }
    ...
    @Override
    public final void onBegin(PlayerCharacter pc) {
        setProgress(pc, onStart(pc));
    }

    public T onStart(PlayerCharacter pc) {
        return null;
    }

    @Override
    public final void onEnd(PlayerCharacter pc) {
        setProgress(pc, onEnd(pc, getProgress(pc)));
    }

    public T onEnd(PlayerCharacter pc, T progress) {
        return progress;
    }

    @Override
    public final void onEvent(Event event, PlayerCharacter pc) {
        setProgress(pc, onEvent(event, pc, getProgress(pc)));
    }
    ...
    @Override
    public final void onUpdate(PlayerCharacter pc, int ticks) {
        setProgress(pc, onUpdate(pc, getProgress(pc), ticks));
    }

    public T onUpdate(PlayerCharacter pc, T progress, int ticks) {
        return progress;
    }
}