...
public class GUI {
    ...
    public Map<UUID, InventoryView> views;
    ...
    public GUI type(InventoryType type) {
        this.type = type;
        this.columns = (type == InventoryType.DISPENSER || type == InventoryType.DROPPER
                || type == InventoryType.HOPPER ? 3 : 9);
     ...
     */
    public void open(Player p, GuiFlag... flags) {
        boolean wait = false;

        for (GuiFlag flag : flags) {
            switch (flag) {
                case NO_PARENTS:
                    RobertStack.clear(p);
                    wait = true;
     ...
     */
    public void close(Player p, GuiFlag... flags) {
        allowClose = true;
