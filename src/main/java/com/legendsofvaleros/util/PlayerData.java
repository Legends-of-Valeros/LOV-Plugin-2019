...
public class PlayerData {
	...
	private static final String PLAYER_RESOURCE_PACK_FORCED = "player_resource_pack_forced";
	...
	static void onEnable() {
		...
		manager.primary(PLAYER_UUID, "VARCHAR(36)")
				...
				.column(PLAYER_RESOURCE_PACK_FORCED, "BOOLEAN")
			...
			manager.query()
					...
					.callback((statement, count) -> {
						...
						if(result.next()) {
							data.username = result.getString(PLAYER_NAME);
							data.resourcePack = result.getString(PLAYER_RESOURCE_PACK);
							data.resourcePackForced = result.getBoolean(PLAYER_RESOURCE_PACK_FORCED);
							data.discordId = result.getString(PLAYER_DISCORD_ID);
						}
						...
						if(currentName != null && !currentName.equals(data.username)) {
							...
							manager.query()
									...
									.values(PLAYER_UUID, uuid.toString(),
											...
											PLAYER_RESOURCE_PACK_FORCED, data.resourcePackForced,
