 ...
 * <br><br>
 * CommandManager manager = new CommandManager(plugin, "Help Tag", "permission node", "command", "aliases"...);
 * @author Stumblinbear
 */
public class CommandManager implements TabCompleter, CommandExecutor {
    ...
    private List<String> getPredicted(Cmd c, String token, int i) {
        ...
        if (i >= cmdArg.length) {
            ...
            if (argNum >= c.argTypes().length)
                ...
                if (!Arg.inst().argInstances.containsKey(c.argTypes()[argNum]))
                    try {
                        Arg.inst().argInstances.put(c.argTypes()[argNum], c.argTypes()[argNum].newInstance());
                    } catch (Exception e) {
                    }
                AbstractArg<?> absArg = Arg.inst().argInstances.get(c.argTypes()[argNum]);
        ...
        try {
            ...
            if (args.length == 0 || args[0].equalsIgnoreCase("help"))
                ...
                if (bestFit != null) {
                    ...
                    if (bestFit.only() != CommandOnly.ALL) {
                        if (sender instanceof Player) {
                    ...
                    for (int j = 0; m.find(); j++) {
                        ...
                        // Cache the instance.
                        if (!Arg.inst().argInstances.containsKey(requestedType))
                            Arg.inst().argInstances.put(requestedType, requestedType.newInstance());
                        AbstractArg<?> absArg = Arg.inst().argInstances.get(requestedType);
                        try {
                    ...
                    Object[] cmdArgsPassed = cmdArgList.toArray(new Object[0]);
    ...
    public CommandFinished displayHelp(CommandSender sender, String[] args) {
        ...
        for (Cmd cmd : commands) {
            ...
            // Should it even show?
            if (cmd.showInHelp() || sender.isOp()) {
                // If it can't be used, don't show it! Simple! :D
                boolean canUse = cmd.permission().equals("") || sender.isOp() || sender.hasPermission(permissionScheme + "." + cmd.permission());

                if (canUse)
                    helpList.add((sender.isOp() && !cmd.showInHelp() ? ChatColor.RED + "!" : "") + ChatColor.GOLD + "/" + command + " " + cmd.cmd() + (cmd.args() != "" ? " " + cmd.args() : "") + ": " + ChatColor.WHITE + (specific ? (cmd.longhelp().equals("") ? cmd.help() : cmd.longhelp()) : cmd.help()) + (sender.isOp() && !cmd.permission().equals("") ? ChatColor.AQUA + " (" + permissionScheme + "." + cmd.permission() + ")" : ""));
            }
        }
    ...
    public static class Arg {
        private static Arg instance;

        public static Arg inst() {
            return instance;
        }

        public HashMap<Class<? extends AbstractArg<?>>, AbstractArg<?>> argInstances;
        ...
        public static class ArgArray extends AbstractArg<List<String>> {
            public List<String> parseArg(String arg) {
                List<String> list = new ArrayList<>(Arrays.asList(arg.split(",")));
                return list;
            }
        ...
        public static class ArgShort extends AbstractArg<Short> {
            public Short parseArg(String arg) {
                return Short.valueOf(arg);
            }

            public String getFailure() {
                return "Argument not a valid short.";
            }
